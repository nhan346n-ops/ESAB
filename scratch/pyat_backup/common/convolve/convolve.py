...
# -----------------------------------------------------------
# This code is an adapted extract from astropy project
# It allow to convolve numpy array taking into account for nan values
#
# The astropy code is released under a bsd-3 licence code and is refered in the /licenses directory
#
# -----------------------------------------------------------
...
try:
    ...
    raise ImportError("Convolution C extension is missing. Try re-building.") from e
...
def _copy_input_if_needed(input_src, dtype=float, order="C", nan_treatment=None, mask=None, fill_value=None):
    # Alias input
    # strip quantity attributes
    if hasattr(input_src, "unit"):
    ...
    try:
        ...
        if nan_treatment == "fill" or np.ma.is_masked(input_src) or mask is not None:
            if np.ma.is_masked(input_src):
                ...
                output = np.array(input_src, dtype=dtype, copy=True, order=order, subok=False)
            ...
            #
            # np.array is deprecated in numpy 2. Replaced with np.asarray.
            # copy=False is not needed, np.asarray has a copy-if-needed behavior
            # 'subok' argument does not exist in np.asarray, so next comment is not relevant.
            ...
            output = np.asarray(
                ...
                dtype=dtype,
                # copy=False,
                order=order,  # subok=True
            )
    except (TypeError, ValueError) as e:
        raise TypeError("input should be a Numpy array or something convertible into a float array", e) from e
    return output
...
def convolve(
    ...
    array_internal = _copy_input_if_needed(
        passed_array, dtype=float, order="C", nan_treatment=nan_treatment, mask=mask, fill_value=np.nan
    )
    ...
    kernel_internal = _copy_input_if_needed(
        passed_kernel, dtype=float, order="C", nan_treatment=None, mask=None, fill_value=fill_value
    )
    ...
    nan_interpolate = (nan_treatment == "interpolate") and (
        np.isnan(array_internal.sum()) or (boundary == "fill" and np.isnan(fill_value))
    )
    ...
    if boundary in ("fill", "extend", "wrap"):
        ...
        if boundary == "fill":
            ...
            if array_internal.ndim == 1:
                ...
                array_to_convolve[
                    pad_width[0] : array_shape[0] + pad_width[0], pad_width[1] : array_shape[1] + pad_width[1]
                ] = array_internal
            else:
                array_to_convolve[
